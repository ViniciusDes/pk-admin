...
const SolidButton = css<ContainerProps>`
  background: ${({ theme, bckColor }) => bckColor || theme.colors.primary};
...
const OutlineButton = css<ContainerProps>`
  ...
  &:hover {
    background: ${({ theme, bckColor }) =>
      bckColor ? transparentize(0.9, bckColor) : transparentize(0.9, theme.colors.primary)};
  }
`;
